...
class PatternIdentificationSpec extends SpecificationWithJUnit {
  ...
  "The pattern identifier" should {
    ...
    "detect splitters of transactions " in {
      ...
      val transactionInfos = List((tx0, 200, 0, 0, 1),
        (tx1, 150, 2, 1, 2),
        (tx2, 50, 3, 1, 3),
        (tx3, 100, 1, 4, 1))
      ...
      tx0.outgoingEdges.filter(_._2.isInstanceOf[TransactionPatternEdge]).size === 2
      tx1.outgoingEdges.filter(_._2.isInstanceOf[TransactionPatternEdge]).size === 0
      tx2.outgoingEdges.filter(_._2.isInstanceOf[TransactionPatternEdge]).size === 0
      tx3.outgoingEdges.filter(_._2.isInstanceOf[TransactionPatternEdge]).size === 0
